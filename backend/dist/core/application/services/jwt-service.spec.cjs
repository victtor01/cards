...
// src/core/application/services/jwt-service.spec.ts
...
describe("jwtService", () => {
  ...
  it("should throw error trying auth", async () => {
    ...
    await globalExpect(jwtService.getSession(RequestMock, ResposeMock)).rejects.toThrow(
      "unauthorized session!"
    );
    globalExpect(jwtService.signJWT).toBeCalledTimes(0);
  });
  it("should thorow error bacause refreshToken is invalid", async () => {
    ...
    import_jose2.jwtVerify.mockRejectedValue(new Error("generic error"));
    await globalExpect(jwtService.getSession(requestMock, responseMock)).rejects.toThrow(
      "session expired!"
    );
    globalExpect(jwtService.renewTokenWithPassportOrThrowError).toBeCalledTimes(1);
    globalExpect(jwtService.signJWT).toBeCalledTimes(0);
    globalExpect(import_jose2.jwtVerify).toBeCalledTimes(2);
  });
});
