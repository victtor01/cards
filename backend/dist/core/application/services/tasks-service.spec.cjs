...
// src/core/application/services/tasks-service.spec.ts
...
var TasksService = class {
  ...
  async parseToTask(data) {
    const parse2 = await CreateTaskSchema.parseAsync({
  ...
  async create(data, userId) {
    const parse2 = await this.parseToTask(data);
    const taskToCreate = new Task({ ...parse2, userId });
  ...
  async updateTask(updateTaskDto, userId) {
    ...
    taskToUpdate.completed = updateTaskDto?.days?.every(
      (value, index2) => value?.toString() === task?.days[index2]?.toString()
    ) ? task.completed : [];
...
var deleteTaskDto = { taskId: "123", userId: "THIS_USER" };
describe("tasks-service", () => {
  ...
  describe("#create", () => {
    it("should create a new task", async () => {
      ...
      globalExpect(tasksRepositoryMock.save).toHaveBeenCalledTimes(1);
      globalExpect(tasksService.parseToTask).toHaveBeenCalledTimes(1);
      globalExpect(created).toBeInstanceOf(Task);
    });
    it("should error in create a new task because of data", async () => {
      ...
      vi.spyOn(tasksService, "parseToTask");
      globalExpect(tasksService.create(copyTask, USERID)).rejects.toThrow();
      globalExpect(tasksRepositoryMock.save).toBeCalledTimes(0);
      globalExpect(tasksService.parseToTask).toBeCalledTimes(1);
    });
  });
  describe("#updateArrayCompleted", () => {
    it("should be an error in updateCompletedTask because the task does not belong to the user", async () => {
      ...
      globalExpect(tasksService.updateArrayCompleted(updatedTaskDataDto)).rejects.toThrow(
        new UnauthorizedException("Usu\xE1rio n\xE3o pode fazer essa a\xE7\xE3o!")
      );
    });
    it("deve filtrar corretamente as datas com base nos dias da semana", async () => {
      ...
      const mockUpdateResult = {
        ...
        raw: [],
        affected: 1
        // Exemplo de resultado da atualização
      };
      ...
      globalExpect(mapSpy).toHaveBeenCalledWith(globalExpect.any(Function));
      globalExpect(tasksRepositoryMock.update).toHaveBeenCalledWith(taskId, {
  ...
  describe("#deleteTask", () => {
    it("should error trying delete task because user not have permission", async () => {
      ...
      globalExpect(response).rejects.toThrow(UnauthorizedException);
      globalExpect(tasksRepositoryMock.findById).toBeCalledTimes(1);
      globalExpect(tasksRepositoryMock.delete).toBeCalledTimes(0);
    });
    it("should error when task not found", async () => {
      tasksRepositoryMock.findById.mockResolvedValueOnce(null);
      const response = tasksService.deleteTask(deleteTaskDto);
      globalExpect(response).rejects.toThrow(NotFoundException);
      globalExpect(tasksRepositoryMock.delete).toBeCalledTimes(0);
    });
    it("should delete task success", async () => {
      const task = new Task({ ...taskMock, userId: deleteTaskDto.userId }, deleteTaskDto.taskId);
      tasksRepositoryMock.findById.mockResolvedValueOnce(task);
      tasksRepositoryMock.delete.mockResolvedValueOnce(await Promise.resolve({}));
      const response = await tasksService.deleteTask(deleteTaskDto);
      globalExpect(response).toBe(true);
      globalExpect(tasksRepositoryMock.delete).toBeCalledTimes(1);
    });
  });
  describe("#findByIdAndUser", () => {
    ...
    it("should success on get task by ID", async () => {
      ...
      globalExpect(tasksRepositoryMock.findById).toBeCalledTimes(1);
      globalExpect(response).toBe(task);
      globalExpect(response).toBeInstanceOf(Task);
    });
  });
  describe("#updateTask", () => {
    it("should give an error trying to update the task because task does not belong to the user", async () => {
      ...
      await globalExpect(tasksService.updateTask(updateTaskDtoMock, userIdMock)).rejects.toThrow(
        UnauthorizedException
      );
      globalExpect(tasksRepositoryMock.findById).toBeCalledTimes(1);
      globalExpect(tasksRepositoryMock.update).toBeCalledTimes(0);
    });
    it("should update task success", async () => {
      ...
      globalExpect(updated).toBeDefined();
      globalExpect(tasksRepositoryMock.findById).toBeCalledTimes(1);
      globalExpect(tasksRepositoryMock.update).toBeCalledTimes(1);
    });
    it("should giva an error trying to update the task because task not exists", async () => {
      ...
      await globalExpect(updated).rejects.toThrow(NotFoundException);
      globalExpect(tasksRepositoryMock.findById).toBeCalledTimes(1);
    });
  });
  describe("#findLates", () => {
    it("should get oldest tasks", async () => {
      const oldestDate = (0, import_dayjs2.default)().subtract(1, "day");
      const tasksMocks = [{ startAt: /* @__PURE__ */ new Date() }, { startAt: oldestDate.toISOString() }];
      const oldestTask = tasksService.getOldestTaskDate(tasksMocks);
      globalExpect(oldestTask).toBeDefined();
      globalExpect(oldestTask).toBe(oldestDate.toISOString());
    });
  });
  describe("#isTaskDueToday", () => {
    it("should show task in day", () => {
      taskMock.startAt = (0, import_dayjs2.default)().subtract(1, "week").format("YYYY-MM-DD");
      taskMock.endAt = (0, import_dayjs2.default)().add(1, "week").format("YYYY-MM-DD");
      taskMock.days = [0, 1, 2, 3, 4, 5, 6];
      taskMock.completed = [];
      taskMock.repeat = "weekly";
      const res = tasksService.isTaskDueTodayLates(taskMock, (0, import_dayjs2.default)().subtract(1, "day"));
      globalExpect(res).toBeDefined();
      globalExpect(res).toBe(taskMock);
    });
  });
});
