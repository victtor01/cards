...
// src/core/application/services/cards-service.spec.ts
var cardsRepositoryMock = {
  save: vi.fn(),
  update: vi.fn(),
...
describe("CardsService", () => {
  let cardsService;
  let findWorksapcesService;
  beforeEach(() => {
    findWorksapcesService = { findOneById: vi.fn() };
  ...
  it("should create a card", async () => {
    ...
    findWorksapcesService.findOneById.mockResolvedValue(mockWorkspace);
    cardsRepositoryMock.save.mockResolvedValue(mockCard);
    const result = await cardsService.create(createCardDto, userId);
    globalExpect(result).toEqual(mockCard);
    globalExpect(cardsRepositoryMock.save).toHaveBeenCalledWith(globalExpect.any(Card));
  });
  it("should throw UnauthorizedException if user does not own workspace", async () => {
    ...
    findWorksapcesService.findOneById.mockResolvedValue(mockWorkspace);
    await globalExpect(cardsService.create(createCardDto, userId)).rejects.toThrow(UnauthorizedException);
  });
  describe("#supress", () => {
    it("should throw error when card not found", async () => {
      const userId = "USERID";
      const cardId = "NOT_FOUND";
      cardsRepositoryMock.findOneById.mockResolvedValue(null);
      await globalExpect(cardsService.supress(userId, cardId)).rejects.toThrow(NotFoundException);
    });
    it("should throw erro when card not belongs to user", async () => {
      ...
      cardsRepositoryMock.findOneById.mockResolvedValue(mockCard);
      await globalExpect(cardsService.supress(userId, cardId)).rejects.toThrow(UnauthorizedException);
    });
  });
});
