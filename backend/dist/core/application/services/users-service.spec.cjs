...
var UsersService = class {
  ...
  async save({ firstName, lastName, email, password }) {
    ...
    const parse2 = await createUserSchema.parseAsync({
      firstName,
      lastName,
      email,
      password
    ...
    const user = new User({
      firstName: parse2.firstName,
      lastName: parse2.lastName,
      email: parse2.email,
      password: parse2.password
    });
    try {
      user.password = await (0, import_bcryptjs.hash)(user.password, 10);
...
// src/core/application/services/users-service.spec.ts
...
describe("usersService", () => {
  ...
  it("should create a new user", async () => {
    createUserSchema.parseAsync.mockResolvedValue(Promise.resolve(userMock));
    ...
    globalExpect(usersRepo.findOneByEmail).toBeCalledTimes(1);
    globalExpect(action).toBeInstanceOf(User);
    globalExpect(action).instanceOf(User);
    globalExpect(action.firstName).toBe("Jonh");
  });
  it("should throw unauthorized error", async () => {
    vi.spyOn(usersService, "findOneByEmail").mockReturnValueOnce(Promise.resolve(userMock));
    globalExpect(usersService.save(dataToCreateNewUser)).rejects.toThrowError();
  });
  describe("#updatePhoto", () => {
    it("should error photo because user does not exist!", async () => {
      ...
      globalExpect(action).rejects.toThrow();
    });
  });
});
